...
def pooling_layer(qc: QuantumCircuit, active_qubits):
    new_active_qubits = [];
    for i in range(0, len(active_qubits) - 1, 2):
        q_control, q_target = active_qubits[i], active_qubits[i+1]; qc.cx(q_control, q_target); new_active_qubits.append(q_target)
    return new_active_qubits
def create_qcnn_ansatz(num_qubits: int):
    num_layers = num_qubits.bit_length() - 1; num_weights = 2 * num_layers; weights_params = ParameterVector('θ', num_weights)
    ansatz = QuantumCircuit(num_qubits, name='QCNN Ansatz'); active_qubits = list(range(num_qubits)); weight_idx = 0
    while len(active_qubits) > 1:
        layer_weights = [weights_params[i] for i in range(weight_idx, weight_idx + 2)]; conv_layer(ansatz, layer_weights, active_qubits); ansatz.barrier()
        active_qubits = pooling_layer(ansatz, active_qubits); ansatz.barrier(); weight_idx += 2
...
class QuantumFunctionAmplitude(Function):
    ...
    def forward(ctx, input_data: torch.Tensor, weights: torch.Tensor, qcnn_ansatz, final_qubit_idx, estimator):
        ...
        weights_np, input_data_np = weights.detach().cpu().numpy(), input_data.detach().cpu().numpy()
...
class QuantumFunctionGeneral(Function):
    ...
    def forward(ctx, input_data: torch.Tensor, weights: torch.Tensor, encoder_circuit, qcnn_ansatz, final_qubit_idx, estimator):
        ...
        input_values, weight_values = [x.detach().cpu().numpy() for x in input_data], [weights.detach().cpu().numpy()] * len(input_data)
        circuits = []
        for x_val in input_values:
            bound_encoder = encoder_circuit.assign_parameters(x_val); full_circuit = bound_encoder.compose(qcnn_ansatz)
    ...
    def backward(ctx, grad_output):
        weights, = ctx.saved_tensors
        encoder_circuit, qcnn_ansatz, input_data, estimator = ctx.encoder_circuit, ctx.qcnn_ansatz, ctx.input_data, ctx.estimator
        weights_np, input_values = weights.detach().cpu().numpy(), [x.detach().cpu().numpy() for x in input_data]
        grad_output_np = grad_output.squeeze().cpu().numpy()
        shift, grad_weights = np.pi / 2, torch.zeros_like(weights)
        for i in range(len(weights)):
            weights_plus, weights_minus = weights_np.copy(), weights_np.copy()
...
class QCNNGeneral(nn.Module):
    def __init__(self, num_qubits: int, encoder_fn, num_input_features: int, num_classes: int = 2, estimator: Estimator = None):
        ...
        self.encoder_circuit = QuantumCircuit(num_qubits, name='Encoder'); encoder_fn(self.encoder_circuit, self.input_params)
        self.qcnn_ansatz, self.q_weights_params, self.final_qubit_idx = create_qcnn_ansatz(num_qubits)
        self.q_weights = nn.Parameter(torch.randn(len(self.q_weights_params))); self.classical_head = nn.Linear(1, num_classes)
    def forward(self, x):
        return self.classical_head(QuantumFunctionGeneral.apply(x, self.q_weights, self.encoder_circuit, self.qcnn_ansatz, self.final_qubit_idx, self.estimator))